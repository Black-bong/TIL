...
- `@SpringBootTest` 스프링부트 통합 테스트를 진행하려면 명시해 줘야한다.
...
- JDBC Template을 사용하기 위해서는 JdbcTemplate을 매개변수 DataSource를 넣어 의존성 주입해줘야 한다.
- 환경설정은 JDBC와 동일하다.
...
public Member save(Member member) {
    ...
    member.sertId(key.longValue());
    return member;
}
```

- 쿼리작성 없이 insert가 가능하다. 메뉴얼에 잘 나와있다.
...
- save 메소드, sql문을 하나도 작성하지 않아도 JPA가 알아서 sql문을 만들고 적용시킨다.
...
- MemberService 클래스에 회원가입(join)메서드가 있으므로 `@Transactional` 으로 데이터 변경이 있음을 명시해줘야 한다.
...
- `@PersistenceContext` 원래 스팩에서는 이걸 필드 주입형태로 작성하여, DI를 해줘야한다고 하는데
...
- `@Autowired` 를 사용한 생성자를 통한 주입을해도 되긴한다.
...
- 따로 빈으로 등록한 MemberRepository가 없을때 위에 만든 SpringDataJapMemberRepository를 찾아서 구현체를 만들고 빈으로 등록
