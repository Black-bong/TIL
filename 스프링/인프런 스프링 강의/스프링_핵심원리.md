...
```
- 위와 같은 로그가 남도록 request스코프를 활용해서 예제를 만들어 보자.
...
```
- 로그를 출력하기 위한 클래스
- `@Scope(value = "request")`를 사용해서 request 스코프로 지정, 이 빈은 스프링 컨테이너 요청 당 하나씩 생성되고, 요청이 끝나는 시점에 소멸한다.
- 빈이 생성되는 시기에 `@PostConstruct`초기화 메서드를 사용해서 `uuid`를 생성해서 저장해둔다.
- 요청마다 새로운 `uuid`가 생성되므로 다른 요청과 구분할 수 있게된다.
- 빈이 소멸되는 시기에 `@PreDestroy`를 사용해 종료 메시지를 출력
```java
@Controller
@RequiredArgsConstructor
...
```
- `@RequestMapping("log-demo")`: `log-demo`라는 URL요청이 들어오면 이 메소드를 맵핑시켜준다.
- `@ResponseBody`: 출력화면 없이 문자를 문자 그대로 출력해줄때 사용
```java
@Service
@RequiredArgsConstructor
...
```
- `myLogger`를 주입 받는 것이 아니라 `myLogger`를 찾을수 있는 DL(dependency lookup)이 주입된다.
```java
@Service
@RequiredArgsConstructor
public class LogDemoService {
...
```
- 필요한 시점에 myLogger를 주입받을 수 있다.
- `ObjectProvider`덕분에 `ObjectProvider.getObject()`를 호출하는 시점까지 스프링 컨테이너에 요청을 지연 시킨다.
